  ...
  return (
    <>
      <div className='bg-primary py-14 px-[1.5rem] sm:px-[3rem] lg:px-[4rem] xl:px-[8rem]'>
        <h2 className='text-dark text-[2rem] font-semibold md:text-4xl mb-8 text-center md:font-bold'>
              ...
              {row.cards.map((card, cardIndex) => (
                ...
                >
                  {card.type === 'text' ? (
                    ...
                    <img 
                      src={card.content} 
                      alt={`Card Image ${cardIndex}`} 
                      className="w-full h-[18rem] md:h-[20rem] lg:h-[18rem] aspect-square object-cover rounded-[15px]" 
                    />
                  )}
                </div>
              ))}
